...
def mirror_atlas(node, dimensions, distance):
    ...
    for i in range(len(node)):
        points = []
        for j in range(
            -(distance // dimensions[i]) - 1, (distance // dimensions[i] + 2)
        ):
...
def solution(dimensions, your_position, trainer_position, distance):
    mirrored = [
        mirror_atlas(your_position, dimensions, distance),
        mirror_atlas(trainer_position, dimensions, distance),
    ]
    res = set()
    angles_dist = {}
        ...
        for j in mirrored[i][0]:
            for k in mirrored[i][1]:
                ...
                if [j, k] != your_position and distance >= l:
                    if (
                        beam in angles_dist and angles_dist[beam] > l
                    ) or beam not in angles_dist:
                        if i == 0:
                            angles_dist[beam] = l
                        else:
                            angles_dist[beam] = l
